...
public:
	...
	Parameter& shortOpt( char shortOpt ){ m_shortOpt = shortOpt; return *this; }
	const string& longOpt() const{ return m_longOpt; }
	Parameter& longOpt( const string& longOpt ){ m_longOpt = longOpt; return *this; }
	...
	Parameter& help( const string& help ){ m_help = help; return *this; }
...
public:
	...
	String& mandatory( bool mandatory ){ m_mandatory = mandatory; return *this; }
	const string& name() const{ return m_name; }
	String& name( const string& name ){ m_name = name; return *this; }
	const string& value() const{ return m_value; }
	String& value( const string& value ){ m_value = value; return *this; }
	string usage() const override{
		std::ostringstream os;
		os<<"-"<<shortOpt()<<" --"<<longOpt()<<" = ";
	...
	void set( const string& arg ) override{
		if( arg.empty() && m_mandatory ){
			throw std::invalid_argument( "Mandatory argument. Value must be supplied." );
		}
		value( arg );
	}
private:
...
public:
	bool mandatory() const{ return m_mandatory; }
	Set& mandatory( bool mandatory ){ m_mandatory = mandatory; return *this; }
	const string& name() const{ return m_name; }
	Set& name( const string& name ){ m_name = name; return *this; }
	const vector<string>& alternatives()const{ return m_alternatives; }
	Set& alternatives( const vector<string>& alternatives ){ m_alternatives = alternatives; return *this; }
	const string& value() const{ return m_value; }
	Set& value( const string& value ){ m_value = value; return *this; }
	string usage() const override{
		std::ostringstream os;
		os<<"-"<<shortOpt()<<" --"<<longOpt()<<" = {";
	...
	void set( const string& arg ) override{
		...
		if( find( m_alternatives.begin(), m_alternatives.end(), arg ) == m_alternatives.end() ){
...
public:
	...
	OptionReader& with( Parameter* p ){ m_opts.push_back( p ); return *this; }
	...
	void parseInput( vector<string> args, size_t i ){
		...
		try{
			if( isOption( val ) ){
				...
				std::cout<<"\t"<<i<<" : "<<nextOptIdx<<" : "<<nextOptIdx-i<<std::endl;
				if( nextOptIdx - i > 2 ){
			...
			throw std::invalid_argument( "Failed to parse option: '"+val+"': " +e.what() );
		}
	}
	...
	void handleOpt( const string& enteredOpt, const string& arg ){
		...
		for( Parameter* opt : m_opts ){
			if( enteredOpt.substr( 2, string::npos ) == opt->longOpt() ||
				...
				std::cout<<"\tmatched:\t'"<<opt->longOpt()<<"' or '"<<opt->shortOpt()<<"'"<<std::endl;
